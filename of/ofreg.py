...
def compute(data_list, results_directory):
    ...
    for item in data_list:
        ...
        with closing(open(item['ult_file'], 'rb')) as ult_file:
            ...
            save_file_template = os.path.basename(item['filebase'])
            save_file = results_directory + os.sep + save_file_template + "_OF.pickle" #item['filebase'].replace('data', 'results') + "_OF.pickle"
            pickle.dump(data, open(save_file, "wb"))
            print("Saving results to %s" % save_file)

            # Compute kinematics for each file in the list and save a CSV file to store the data
            scaling = 1.0
            ult_period = ultra_time[1] - ultra_time[0]
            x_range = range(1, ultra_interp[0].shape[0])
            y_range = range(1, ultra_interp[0].shape[1])
            xx, yy = np.meshgrid(x_range, y_range)

            x_indices, y_indices = np.meshgrid(np.arange(0, xx.shape[0]), np.arange(0, xx.shape[1]))
            pos, vel = compute_kinematics(ofdisp, ultra_time, ult_no_frames, x_indices, y_indices, ult_period, scaling)

            df = pandas.DataFrame({"time" : ultra_time[:-1], "pos_x" : pos[:, 0], "pos_y" : pos[:, 1], "vel_x" : vel[:, 0], "vel_y" : vel[:, 1]})
            df.to_csv(results_directory + os.sep + save_file_template + '_kinematic_data.csv', index=False)
